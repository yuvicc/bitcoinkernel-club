...
int main() {
    ...
    printf("Creating transaction object from raw bytes...\n");
    btck_Transaction* transaction_ptr =
        btck_transaction_create(tx, sizeof(tx));
    printf("Transaction created successfully!\n\n");

    printf("Creating script pubkey object...\n");
    btck_ScriptPubkey* script_pubkey_ptr =
        btck_script_pubkey_create(script, sizeof(script));
    ...
    btck_TransactionOutput* output =
        btck_transaction_output_create(script_pubkey_ptr, amount);
    const btck_TransactionOutput* output_ = output;
    printf("Transaction output created!\n\n");

    printf("Creating precomputed transaction data...\n");
    btck_PrecomputedTransactionData* precomputed_data_ptr =
        btck_precomputed_transaction_data_create(transaction_ptr, &output_, 1);
    ...
    btck_precomputed_transaction_data_destroy(precomputed_data_ptr);
    btck_transaction_output_destroy(output);
    btck_script_pubkey_destroy(script_pubkey_ptr);
    btck_transaction_destroy(transaction_ptr);
